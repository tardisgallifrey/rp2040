#if !defined(REQUIRED_INCLUDES_FOR_OLEDI2C)
#define REQUIRED_INCLUDES_FOR_OLEDI2C
#include "pico/stdlib.h"
#include "pico/binary_info.h"
#include "hardware/i2c.h"
#endif // REQUIRED_INCLUDES_FOR_OLEDI2C
...
/*
    _u(xx...) -- The _u is merely a method to further define the value
...
#define OLED_IGNORE_RAM _u(0xA5)
...
#define OLED_ADDR _u(0x3C)          /*CRITICAL VALUE, I2C address of module */
#define OLED_HEIGHT _u(32)          /*CRITICAL VALUE, height, in pixels of display */
#define OLED_WIDTH _u(128)          /*CRITICAL VALUE, width, in pixels of display  */
#define OLED_PAGE_HEIGHT _u(8)      /*All OLED pages have a height of 8   */
...
//Structure used to define the basic OLED screen
struct render_area {
    uint8_t start_col;
    uint8_t end_col;
    uint8_t start_page;
...
void i2c_initialize(void){
    ...
    //This block must be set first.
    //400 * 1000 is a way of setting baud to 400Khz
...
//I am assuming this gets defined in hardware/i2c.h
//probably some kind of safety, but doesn't really do anything here.
//Keeping in comment for future possibility
//#ifdef i2c_default
...
void oled_send_cmd(uint8_t cmd) {
    ...
    //This is the basic write to the OLED code
    //A 2 item array that contains 0x80 at buf[0].
    //and the command number at buf[2]
    uint8_t buf[2] = {0x80, cmd};

    //The actual writing, i2caddr, device address bit-anded with write or read mode,
    //the 2 element buffer, a 2, and false.

    //We defined OLED_ADDR, OLED_WRITE_MODE above in the defines
    i2c_write_blocking(i2c_default, (OLED_ADDR & OLED_WRITE_MODE), buf, 2, false);
}


//Send a whole data buffer at once
//Similar to send command, but just for data to pixels
void oled_send_buf(uint8_t buf[], int buflen) {
    ...
    //Copies our buf array to temp_buf array
    //but starts at item 1 instead of item 0.
    //Puts buf[0] at temp_buf[1]
    for (int i = 1; i < buflen + 1; i++) {
        temp_buf[i] = buf[i - 1];
    }

    //Now, we can put the control byte (0x40)
    //at temp_buf[0] where it belongs
...
void oled_init() {
    ...
    //OLED_SET_DISP is bit-OR'ed with 0x00
